...
### 批量清理失效引用
```
POST /api/files/clean-invalid-references
Headers: Authorization: Bearer <token>
...
- 私人空间显示锁图标
- **失效引用显示**：
  - 失效的引用（`isInvalid: true`）显示为灰色 + 删除线
  - 显示“（已失效）”标签
  - 提供“删除”按钮
  - 添加“清理失效引用”按钮（批量删除）
...
// 显示失效引用
files.forEach(file => {
  if (file.type === 'reference' && file.isInvalid) {
    // 添加灰色 + 删除线样式
...
// 清理失效引用
const { data } = await axios.post('/api/files/clean-invalid-references', {}, {
  headers: { Authorization: `Bearer ${token}` }
})
alert(data.message) // "已清理 3 个失效引用"
```
